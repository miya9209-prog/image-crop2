...
DEFAULT_REP_CUTS_PER_PRODUCT = 5
...
def get_product_no_from_url(url: str) -> str:
    ...
    return m.group(1) if m else safe_filename(guess_base_name_from_url(url))


def pick_product_name_from_html(html: str, product_no: str = "") -> str:
    """Cafe24 상품 상세 HTML에서 상품명을 최대한 안전하게 추출."""
    soup = BeautifulSoup(html, "html.parser")
    ...
    for sel, attr in meta_selectors:
        node = soup.select_one(sel)
        val = (node.get(attr) if node else "") or ""
        val = re.sub(r"\s+", " ", val).strip()
        if val:
            val = re.split(r"\s*[|:-]\s*미샵|\s*[|:-]\s*MISHARP", val, flags=re.I)[0].strip()
            return safe_filename(val)

    title_node = soup.select_one("title")
    if title_node:
        val = re.sub(r"\s+", " ", title_node.get_text(" ", strip=True)).strip()
        val = re.split(r"\s*[|:-]\s*미샵|\s*[|:-]\s*MISHARP", val, flags=re.I)[0].strip()
        if val:
            return safe_filename(val)

    return f"product_{product_no}" if product_no else "product"


def build_cut_items_from_source_images(
    source_images: List[Image.Image],
    auto_exclude_text: bool = True,
    start_idx: int = 1,
) -> List[CutItem]:
    """상품 1개 단위로 상세이미지를 컷 후보로 변환."""
    items: List[CutItem] = []
    ...
    for img in source_images:
        ...
        for cut in extracted:
            ...
            items.append(CutItem(idx=global_idx, pil=cut, excluded_auto=ex, reason=reason))
            global_idx += 1

    return items


def representative_cut_score(item: CutItem, order_idx: int) -> float:
    """
    모델 피팅컷으로 보이는 이미지를 우선하기 위한 단순 점수.
    - 세로형/큰 이미지/상단 순서 컷 우선
    - 텍스트·로고 추정 컷은 강하게 감점
    """
    w, h = item.pil.size
    if w <= 0 or h <= 0:
    ...
    score += min(area / (900 * 1200), 2.2) * 35
    if 1.0 <= aspect <= 2.2:
        score += 35
    elif 0.75 <= aspect < 1.0:
        score += 12
    elif aspect > 2.2:
        score -= 18
    else:
    ...
    # 상세페이지 초반부에 모델 착용컷이 많다는 미샵 구조 반영
    score += max(0, 25 - order_idx * 2.5)

    if item.excluded_auto:
        score -= 120
    return score


def select_representative_items(items: List[CutItem], limit: int = DEFAULT_REP_CUTS_PER_PRODUCT) -> List[CutItem]:
    """상품별 대표 착용컷 limit장 자동 선별."""
    if not items:
        return []

    candidates = [it for it in items if not it.excluded_auto]
    if not candidates:
        candidates = items[:]

    scored = []
    for order_idx, it in enumerate(candidates):
        scored.append((representative_cut_score(it, order_idx), order_idx, it))

    scored.sort(key=lambda x: (-x[0], x[1]))
    selected = [it for _, _, it in scored[:limit]]

    # 화면 확인이 자연스럽도록 원래 상세페이지 등장 순서로 재정렬
    order_map = {id(it): i for i, it in enumerate(items)}
    selected.sort(key=lambda it: order_map.get(id(it), 9999))
    return selected


def fetch_representative_group_from_product_url(
    product_url: str,
    per_product_limit: int = DEFAULT_REP_CUTS_PER_PRODUCT,
    auto_exclude_text: bool = True,
) -> ProductCutGroup:
    product_url = normalize_url(product_url)
    product_no = get_product_no_from_url(product_url)

    try:
        html = fetch_html(product_url)
        product_name = pick_product_name_from_html(html, product_no=product_no)
        candidates = pick_body_image_urls_from_html(product_url, html)
        ...
        if not downloaded:
            return ProductCutGroup(product_no, product_name, product_url, [], True, "상세 이미지 다운로드 실패")

        # 기존 fetch_detail_images_from_product_url의 후보 선별 기준과 동일하게 유지
        long_imgs = []
        for im in downloaded:
            w, h = im.size
            if h > w * 2 and h > 2000:
        ...
        if not long_imgs:
            long_imgs = downloaded[:30]

        cut_items = build_cut_items_from_source_images(long_imgs, auto_exclude_text=auto_exclude_text)
        selected = select_representative_items(cut_items, limit=per_product_limit)

        if not selected:
            return ProductCutGroup(product_no, product_name, product_url, [], True, "대표컷 후보 없음")

        # 상품별 번호는 1부터 다시 부여
        for i, item in enumerate(selected, start=1):
            item.idx = i
            item.excluded_manual = False

        return ProductCutGroup(product_no, product_name, product_url, selected, False, "")

    except Exception as e:
        return ProductCutGroup(product_no, f"product_{product_no}", product_url, [], True, str(e))


def build_representative_groups_incremental(
    product_urls: List[str],
    per_product_limit: int = DEFAULT_REP_CUTS_PER_PRODUCT,
    auto_exclude_text: bool = True,
    progress_bar=None,
    progress_text=None,
) -> List[ProductCutGroup]:
    groups: List[ProductCutGroup] = []
    ...
    for i, u in enumerate(product_urls, start=1):
        ...
        if progress_text is not None:
            progress_text.text(f"대표컷 추출 중... {i}/{total}")

        group = fetch_representative_group_from_product_url(
            u,
    ...
    if progress_text is not None:
        success_count = sum(1 for g in groups if not g.failed and g.items)
        progress_text.text(f"대표컷 추출 완료: 성공 {success_count}개 / 전체 {len(groups)}개")
...
with colA:
    input_type = st.radio("입력 선택", ["상품 URL", "카테고리 URL", "카테고리 대표컷 추출", "상세페이지 JPG 업로드"], horizontal=True)
    ...
    elif input_type in ["카테고리 URL", "카테고리 대표컷 추출"]:
        category_url = st.text_input(
            "미샵 카테고리 URL",
            placeholder="https://misharp.co.kr/product/list.html?cate_no=XXX",
        )
        if input_type == "카테고리 대표컷 추출":
            st.markdown(
                '<div class="small-note">※ 신규 기능: 카테고리 상품을 수집한 뒤 <b>상품별 모델 피팅 대표컷 5장</b>을 자동 선별합니다. 검수 후 상품별 폴더 ZIP으로 저장됩니다.</div>',
                unsafe_allow_html=True,
            )
            per_product_limit = st.number_input(
                "상품당 대표컷 수",
                min_value=1,
                max_value=10,
                value=5,
                step=1,
                help="기본은 상품당 5장입니다. 모델 전신/상체/하체 착용컷 중심으로 자동 선별합니다.",
            )
        else:
            st.markdown(
    ...
    if input_type in ["상품 URL", "카테고리 URL", "카테고리 대표컷 추출"]:
...
run_label = "✅ 카테고리 대표컷 추출하기" if input_type == "카테고리 대표컷 추출" else "✅ 본문 상품컷 추출하기"
    ...
    try:
        with st.spinner("이미지 수집/분석 중..."):
            ...
            if input_type == "상품 URL":
                ...
                base_name = f"category_representative_{len(product_urls)}products"
                progress_bar = st.progress(0.0)
                progress_text = st.empty()
                product_groups = build_representative_groups_incremental(
                    product_urls,
                    per_product_limit=int(per_product_limit),
                    auto_exclude_text=auto_exclude_text,
                    progress_bar=progress_bar,
                    progress_text=progress_text,
                )
                ...
                success_groups = [g for g in product_groups if (not g.failed and g.items)]
                if not success_groups:
                    st.error("카테고리 상품들에서 대표컷을 찾지 못했어요. 디버그 정보를 켜고 확인해 주세요.")
                    st.stop()

                st.session_state["cuts_base_name"] = base_name
                st.session_state["cuts_crop_mode"] = crop_mode
                st.session_state["product_groups"] = product_groups
                st.session_state["last_input_url_count"] = len(product_urls)
                st.session_state["last_total_found_product_urls"] = original_count
                st.session_state["last_truncated_by_limit"] = truncated_by_limit
                st.session_state.pop("cuts_items", None)
                st.session_state.pop("dl_zip", None)
                st.session_state.pop("dl_zip_name", None)

                st.success(f"대표컷 추출 완료! 성공 상품 {len(success_groups)}개 / 전체 {len(product_groups)}개")
                st.info(f"카테고리에서 찾은 상품: {original_count}개 / 처리 대상: {len(product_urls)}개")
                if truncated_by_limit:
                    st.warning("최대 처리 상품 수 제한으로 일부 상품만 처리했습니다.")
                if debug_mode:
                    failed = [f"{g.product_no} / {g.product_url} / {g.reason}" for g in product_groups if g.failed]
                    if failed:
                        st.code("\n".join(failed))
                st.stop()
            ...
            st.session_state.pop("product_groups", None)
...
if "product_groups" in st.session_state:
    base_name = st.session_state.get("cuts_base_name", "category_representative")
    crop_mode = st.session_state.get("cuts_crop_mode", "이미지 그대로 자르기")
    groups: List[ProductCutGroup] = st.session_state.get("product_groups", [])

    success_groups = [g for g in groups if (not g.failed and g.items)]
    failed_groups = [g for g in groups if g.failed or not g.items]
    total_candidate = sum(len(g.items) for g in success_groups)

    st.markdown(
        f"""
<div class="card">
<b>카테고리 대표컷 추출 상태</b><br/>
- 성공 상품: <b>{len(success_groups)}개</b><br/>
- 실패/대표컷 없음: <b>{len(failed_groups)}개</b><br/>
- 대표컷 후보: <b>{total_candidate}개</b><br/>
- 자르기 모드: <b>{crop_mode}</b><br/>
- 저장 방식: <b>상품별 폴더 ZIP</b>
</div>
""",
        unsafe_allow_html=True,
    )

    if failed_groups and debug_mode:
        st.write("### 실패 상품")
        st.code("\n".join([f"{g.product_no} / {g.product_name} / {g.reason} / {g.product_url}" for g in failed_groups]))

    st.markdown('<div class="hr"></div>', unsafe_allow_html=True)
    st.write("### 상품별 대표컷 검수")
    st.caption("필요 없는 컷만 체크해서 제외하세요. ZIP은 체크되지 않은 컷만 상품별 폴더로 저장합니다.")

    for gi, group in enumerate(success_groups, start=1):
        folder_name = safe_filename(f"{group.product_no}_{group.product_name}")
        with st.expander(f"{gi}. {group.product_name} / 상품번호 {group.product_no} / 대표컷 {len(group.items)}장", expanded=(gi <= 3)):
            st.caption(group.product_url)
            cols = st.columns(5)
            for ii, item in enumerate(group.items, start=1):
                key = f"rep_ex_{base_name}_{group.product_no}_{ii}"
                ...
                with cols[(ii - 1) % 5]:
                    st.image(thumb, caption=f"#{ii} ({item.pil.size[0]}x{item.pil.size[1]})", use_container_width=True)
                    st.checkbox("이 컷 제외", key=key)
    ...
    st.write(f"최종 포함 대표컷: **{included_count}개**")

    if included_count == 0:
        st.warning("포함된 대표컷이 0개입니다. 제외 체크를 해제해 주세요.")
    else:
        if st.button("📦 상품별 폴더 ZIP 만들기", use_container_width=True):
            with st.spinner("상품별 ZIP 생성 중..."):
                ...
                for group in success_groups:
                    ...
                    for ii, item in enumerate(group.items, start=1):
                        ...
                        if st.session_state.get(key, False):
                            continue
                        out = apply_crop_mode(item.pil, crop_mode)
                        fname = f"{folder_name}/{n:02d}.jpg"
                        files.append((fname, pil_to_bytes_jpg(out, quality=95)))
                        n += 1

                zip_bytes = make_download_zip(files)
                st.session_state["dl_zip"] = zip_bytes
                st.session_state["dl_zip_name"] = f"{safe_filename(base_name)}_representative_cuts.zip"

        if st.session_state.get("dl_zip"):
            st.download_button(
                "⬇️ 대표컷 ZIP 다운로드",
                data=st.session_state["dl_zip"],
                file_name=st.session_state.get("dl_zip_name", f"{safe_filename(base_name)}_representative_cuts.zip"),
                mime="application/zip",
                use_container_width=True,
                key=f"download_rep_zip_{base_name}_{crop_mode}",
            )
...
elif "cuts_items" not in st.session_state:
    st.info("먼저 위에서 **‘본문 상품컷 추출하기’** 또는 **‘카테고리 대표컷 추출하기’**를 실행해 주세요.")
else:
